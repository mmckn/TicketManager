...
const App = () => {
  ...
  // On startup get the number of tickets stored in the API - used to work out number of pages.
    useEffect(() => {ticketsService.getAmount().then( numberOfPages =>{
...
.catch(err =>{ setErrorMessage('error getting tickets from the the server')})
}, [])
...
const findId =(event)=>{
 ...
 if(ticketLocal.length > 0){
  ...
  .then(serverTickets => {
    // The API returns and empty array if the id is not found.
    if(serverTickets.data.results.length === 0){
      setErrorMessage('There is not ticket matching this Id in the database')
    }
    else{
